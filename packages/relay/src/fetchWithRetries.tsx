...
function fetchWithRetries(uri: string, initWithRetries?: InitWithRetries): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
        ...
        .then((response) => {
          ...
          if (isRequestAlive) {
            ...
            if (response.status >= 200 && response.status < 300) {
              ...
              // TODO: Only retry on transient HTTP errors.
              // eslint-disable-next-line
              console.log(false, 'fetchWithRetries: HTTP error, retrying.'), retryRequest();
            } else {
              // Request was not successful, giving up.
              const error: any = new Error(
                `fetchWithRetries: Still no successful response after ${requestsAttempted} retries. Try again later.`,
              );
    ...
    function shouldRetry(attempt: number): boolean {
      return ExecutionEnvironment.isServer && attempt <= _retryDelays.length;
    }
